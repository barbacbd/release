...
BOOTSTRAP_BUCKET="${INFRA_ID}-bootstrap-ignition"
if verify_bucket_encryption "${BOOTSTRAP_BUCKET}" "Bootstrap ignition"; then
  echo "$(date -u --rfc-3339=seconds) - Bootstrap bucket validation: PASSED"
else
  echo "$(date -u --rfc-3339=seconds) - WARNING: Bootstrap bucket validation failed (this may be expected if bootstrap is complete)"
fi
...
if echo "${REGISTRY_CONFIG}" | jq -e '.spec.storage.gcs.keyID' &>/dev/null; then
